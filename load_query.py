...
import os
import urllib.request, urllib.parse, urllib.error #for parsing the resides from the query file. Optional
import tempfile
import sys
...
USAGE
    ...
    load_query <query_file> [, receptor = True [, ligand = True [, loadall = True[, trans = 0.7 [, mesh = True [, as_one = False]]]]]
...
PYTHON API
    ...
    colorlist = { 'Hydrophobic':        [0.100, 1.000, 0.000],
                  ...
                  'TryptophanAnalog':   [1.0, 1.0, 0.0],\
                  'TyrosineAnalog':     [1.0, 1.0, 0.0]\
                  }
    ...
    #            print 'Receptor file saved:', tmpname
                ...
                cmd.load(tmpname, recfname, zoom=0)
    ...
    if LIGAND:
        ...
        if 'sdf' in pts:
            if pts.get('sdf') not in  ['',None]:
                ...
                #load it into pymol
                cmd.load(tmpname, 'residues', zoom=0)
                #show as sticks
        ...
        for p in pts.get('points'):
            ...
            if LOADALL or enab: 
                ...
                if USE_MESH:
                    ...
                    spherelist = [ALPHA, TRANS, COLOR] + colors + [SPHERE, x, y, z, rad]
...
#                spherelist += [END]
                ...
                if 'svector' in p:
                    if 'vector_on' in p:
                        #see if the vector is turned on
                        vec_on = p.get('vector_on')
                        
                        #if it's on
                        if vec_on != 0:
                            
                            #get the vector of the arrot
                            svec = p.get('svector')
                            if DEBUG > 3:
                                print('svec', svec)
                            if svec != None:
                ...
                if USE_MESH:
                    cgo_list = spherelist + [END] + arrowlist
                else:
                    cgo_list = spherelist + arrowlist
                
                if DEBUG > 3:
                    print('spherelist:', len(spherelist))
                    print('cgo_list:', len(cgo_list))
                    print('arrowlist:', len(arrowlist))
                ...
                if AS_ONE:
                    if enab:
                        whole_query.extend(cgo_list)
                else:
                    cmd.load_cgo(cgo_list, dispname, 1, zoom =0)
                    if not enab:
    ...
    if AS_ONE:
        if DEBUG > 4:
            print('whole_query')
            print(whole_query)
        cmd.load_cgo(whole_query, os.path.basename(query_file),1, zoom=0)
...
#        parent.mainloop()
        ...
        if DEBUG > 3:
            ...
            for key,val in self.display_options.items():
                print(key, val)
        
        
    
    def load_query_dialog(self, app):
        file = tkinter.filedialog.askopenfilename(title = 'Open query file...',
